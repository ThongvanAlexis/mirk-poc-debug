...
- [ ] **FOG-14**: A higher-fidelity user-observable noise-pattern stability regression test that catches the symptom space FOG-11 missed. Candidate test designs: (a) per-pixel visual diff against a reference frame after `MapController.move(...)` keeps pixelOriginX / size.width below a precision-budget threshold (e.g., 1024 — keeps fp32 fract precision sane); (b) shader-input-budget assertion that the maximum `uPixelOrigin / uResolution` ratio stays under a precision-safe magnitude at ANY supported zoom (zoom 18 → 4.3M pixelOrigin → ratio 110k breaches; zoom 15 → 4.3M → ratio 11k breaches; zoom 13 → 1M → ratio 2700 OK); (c) integration test asserting the painter's pixelOrigin uniform decomposes into integer + fractional parts CPU-side so the shader sees a stable [0, 1] range. Catches the persisting Walk #2 symptom (developer's *"rotating 90° very fast / translated very fast repetitively"* per `03.1-FALSIFICATION-2.md` sub-section B) that FOG-11's raw-uniform-delta-magnitude assertion missed. _(Phase 3.1 — Plan 03.1-07 pending)_
...
| FOG-13 | Phase 3.1 | Pending (P03.1-07) — fog-rect viewport-coverage symmetric compensation; surfaced by P03.1-06 Walk #2 |
| FOG-14 | Phase 3.1 | Pending (P03.1-07) — higher-fidelity user-observable noise-pattern stability test; surfaced by P03.1-06 Walk #2 |
...
- Phase 3.1 (Fix Fog Pan-Translation): 9 requirements (FOG-09, FOG-10, FOG-11, FOG-12, FOG-13, FOG-14, PERF-07, PERF-08, UX-01)
