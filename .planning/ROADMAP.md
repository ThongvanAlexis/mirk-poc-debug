...
**Requirements**: FOG-09, FOG-10, FOG-11, FOG-12, FOG-13, FOG-14, FOG-15, FOG-16, FOG-17, FOG-18, FOG-19, PERF-07, PERF-08, UX-01, UX-02, DEBUG-01, DEBUG-02 (DEBUG-01 + FOG-15 added 2026-05-03 via Plan 03.1-07; FOG-14 refined; FOG-16 + FOG-17 + UX-02 added 2026-05-03 via Plan 03.1-09 Walk #3; FOG-18 + FOG-19 + DEBUG-02 added 2026-05-04 via Plan 03.1-12 Walk #4 follow-up Dart-only fix bundle)
**Depends on**: Phase 3
**Plans**: 11 plans landed + Plans 03.1-12 + 03.1-13 planned 2026-05-04 via /gsd:plan-phase 3.1 --gaps (Walk #4 closed `iterating-with-major-progress` 2026-05-04; per CONTEXT §Iteration policy no hard cap on walk count — Plan 03.1-12 Dart-only fix bundle + Plan 03.1-13 Walk #5 pending execution)
  ...
  - [ ] 03.1-12-PLAN.md — Dart-only fix bundle (Plan 03.1-12+ corrected candidate set per 03.1-FALSIFICATION-4.md §Supplementary Walk Evidence): C1' eliminate FOG-17a integer-wrap-modulo (introduce **FOG-18** — Walk #4 debug-spiral positive control proved noise function not truly periodic on kPocFogNoiseTilePx; the wrap event IS the bug not the precision penalty); C2' replace camera.pixelOrigin with zoom-invariant lat/lng-anchored basis worldPxAtRefZoom * pow(2, zoom - refZoom) (introduce **FOG-19** — fixes Q1b zoom-gesture O(1M) raw-px snap between zoom integer levels); C3' remove MapOptions.cameraConstraint (introduce **DEBUG-02** stress-test diagnostic — developer pans ~100 km to verify FOG-18 + FOG-19 are precision-clean at extreme world coordinates). Three atomic commits, one per axis, for post-walk attribution. ZERO shader source edits (MIRL constraint per CLAUDE.md `# MIRL solution`). Files: lib/config/constants.dart + lib/presentation/widgets/fog_layer.dart + lib/presentation/screens/map_screen.dart + 4 test files (rewrite + 1 new) + REQUIREMENTS.md.
  - [ ] 03.1-13-PLAN.md — Pre-walk gates + iPhone 17 Pro UAT Walk #5 + `03.1-FALSIFICATION-5.md` + verdict authoring (mirrors Plan 03.1-11 three-task structure: auto pre-walk + checkpoint:human-verify sideload + auto verdict authoring). Walk #5 validates Plan 03.1-12 Dart-only fix bundle: C1' MAX-zoom sustained pan regime (≥ 30 sec) + C2' deliberate zoom in/out transitions (≥ 5) + C3' ~100 km pan stress-test. Verdict reverses to CONFIRMED-AFTER-FIX (Phase 4/5 unblock; Plan 03-08 DENIED reversed) OR strengthens to DENIED-FINAL OR opens 6th ITERATING (Plan 03.1-14+ pending).
...
| 03.1. Fix Fog Pan-Translation | 11/13 | In Progress (5th iteration, Plan 03.1-12 Dart-only fix bundle + Plan 03.1-13 Walk #5 pending execution) |  |
