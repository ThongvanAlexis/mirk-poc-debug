...
**Depends on**: Phase 3
**Plans**: 11 plans landed + Plans 03.1-12 (2-task Dart-only fix bundle, C2 deferred) + 03.1-13 (Walk #5) planned 2026-05-04 via /gsd:plan-phase 3.1 --gaps (Walk #4 closed `iterating-with-major-progress` 2026-05-04; per CONTEXT §Iteration policy no hard cap on walk count — Plan 03.1-12 reduced from 3 tasks to 2 tasks post-plan-checker review 2026-05-04 with C2 / FOG-19 zoom-invariant world-coordinate basis DEFERRED to follow-up plan after /gsd:discuss-phase 3.1 resolves the dimensional-mismatch design question; Plan 03.1-13 Walk #5 pending execution)
  ...
  - [ ] 03.1-13-PLAN.md — Pre-walk gates + iPhone 17 Pro UAT Walk #5 + `03.1-FALSIFICATION-5.md` + verdict authoring (mirrors Plan 03.1-11 three-task structure: auto pre-walk + checkpoint:human-verify sideload + auto verdict authoring). Walk #5 validates Plan 03.1-12 Dart-only fix bundle (C1 + C3 only — C2 deferred): C1' MAX-zoom sustained pan regime (≥ 30 sec) + C3' ~50-100 km extreme-distance pan stress-test under removed cameraConstraint + Q1b zoom-gesture OBSERVATION capture (≥ 5 deliberate zoom in/out transitions; verbatim observation feeds the C2 follow-up plan; Q1b is NOT verdict-determining since FOG-19 is deferred). Verdict outcomes: **CONFIRMED-AFTER-FIX-PARTIAL** (NEW intermediate verdict; Q1 + C3 closed but Q1b residual still present awaiting C2 follow-up plan + Walk #6; Phase 3.1 stays In Progress) OR **CONFIRMED-AFTER-FIX (FULL)** (very unlikely — would require Q1b to spontaneously resolve with no C2 fix) OR **DENIED-FINAL** (Q1 SNAP persists at MAX zoom even with wrap eliminated) OR **ITERATING** (NEW failure mode beyond Q1b — Plan 03.1-14+ pending).
...
| 03.1. Fix Fog Pan-Translation | 11/13 | In Progress|  |
